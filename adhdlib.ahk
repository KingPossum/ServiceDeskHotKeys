...
Hotstring(trigger, label, mode := 1, clearTrigger := 1, cond := ""){
	...
	newline :="!n"
	changedlist := []
	static keysBound := false,hotkeyPrefix := "~$", hotstrings := {}, typed := "", keys := {"symbols": "!""#$%&'()*+,-./:;<=>?@[\]^_``{|}~", "num": "0123456789", "alpha":"abcdefghijklmnopqrstuvwxyz", "other": "BS,Return,Tab,Space", "breakKeys":"Left,Right,Up,Down,Home,End,RButton,LButton,AppsKey,Rwin,WheelDown,WheelUp,f1,f2,f3,f4,f5,f6,f7,f8,f9,f6,f7,f9,f10,f11,f12", "numpad":"Numpad0,Numpad1,Numpad2,Numpad3,Numpad4,Numpad5,Numpad6,Numpad7,Numpad8,Numpad9,NumpadDot,NumpadDiv,NumpadMult,NumpadAdd,NumpadSub,NumpadEnter"}, effect := {"Return" : "`n", "Tab":A_Tab, "Space": A_Space, "Enter":"`n", "Dot": ".", "Div":"/", "Mult":"*", "Add":"+", "Sub":"-"}
			...
			if (RegExMatch(typed, matchRegex, local$)){
				...
				if (IsLabel(v.label)){
					...
					Global toSendFinal
						...
						;toSendFinal := StrReplace(toSend, "!c", clipget())
						;OnClipboardChange("clipboardchanged")

						;MsgBox, % this is a test          toSendFinal                                       clipget() 
						;MsgBox,%toSendFinal%
						;MsgBox, %toSend%
						If InStr(toSend,"ERROR", true)
							return
						else
							clipboardtrigger := "{!}c"
							currentdatetrigger := "{!}currentdate" 
							aa := A_DD . "-" . A_MMMM . "-" . A_YYYYy
							proc1 := StrReplace(toSend,clipboardtrigger,Clipboard)
							proc2 := StrReplace(proc1,currentdatetrigger, aa)
				
						SendInput, % proc2	
						;SendInput, %  StrReplace(toSend, "{!}c", Clipboard)
						return
						;toSend, "!c", clipval    StrReplace(toSend, "!c", clipval) 
...
} ;END OF HOTSTRING XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
 ...
 ; Ensures a consistent starting directory.
		...
		dir = C:\Users\%A_UserName%\AppData\Roaming\SDHK
		If !FileExist(dir) {
			FileCreateDir, %dir%
		If ErrorLevel
	...
	create_gui(){
		...
		hotkey_w := this.private.gui_w - 155
		...
		Gui, Add, Link,x5 y%current_row%, ServiceDeskHotKeys intercepts user-defined hotkeys and hotstrings, 
		Gui, Add, Link,x5 yp+25,By Rasheed Peters | rasheedpeters@llu.edu x54083
		...
		;local tmp
		;tmp := w - 90
		;Gui, Add, Button, x%tmp% y10 vadhd_debug_window gadhd_debug_window_change, Show Window
...
join( strArray ){
  ...
  return substr(s, 3)
				...
				clipboardchanged(){
					global changedlist
					changedlist.Push(clipboard)
					MsgBox, % join(changedlist)
					return
						}
						
clipget(){
	string := Clipboard
	MsgBox, ive been called
	MsgBox, % string
	return string
}